...
pub async fn command(_args: &Args) {
    ...
    let task_choices: Vec<String> = tasks.iter().map(|task| {
        format!(
            "\nDescription: {}\nDifficulty: {}\nPriority: {}\nDeadline: {}",
            task.description,
            task.difficulty.as_ref().map_or("Not set".to_string(), |d| d.to_string()),
            task.priority.as_ref().map_or("Not set".to_string(), |p| p.to_string()),
            task.deadline.map_or("Not set".to_string(), |d| d.to_string())
        )
    }).collect();

    let task_description = inquire::Select::new(
